    ...
    <div class="card-body pt-0">
      ...
      >
        ...
        <template v-slot:cell-ssmFees="{ row: customer }">
          <span class="text-success">{{ customer.ssmFee }}</span>
        </template>
        <template v-slot:cell-serviceFees="{ row: customer }">
          <span class="text-primary"> {{ customer.serviceFee }}</span>
        </template>
        <template v-slot:cell-edit="{ row: customer }">
          <div
            class="btn btn-sm btn-light text-gray-600 btn-active-light-primary"
          >
            <router-link
              :to="`/product-management/EditProductListing/${customer.id}`"
  ...
  setup() {
    ...
    onMounted(async () => {
      ...
      try {
        ...
        await ApiService.get(`${process.env.VUE_APP_API_URL}/v1/products`).then(
          ({ data }) => {
            loading.value = false;
